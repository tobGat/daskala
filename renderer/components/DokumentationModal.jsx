...
const DOKU = [
  ...
  },
  {
    id: 'schueler', titel: 'Schüler:innen', bloecke: [
      ...
      { h: 'Klassenliste erstellen (druckbares PDF)' },
      'Der Button „📋 Klassenliste" in der Schüler:innen-Verwaltung öffnet einen Generator für druckbare Listen (PDF). Zwei Arten: „Klassenliste" mit Namen (du wählst per Häkchen, welche Informationen als Spalten erscheinen – z. B. Nachname, Vorname, Geburtsdatum, Klasse, Telefon, Adresse, Merkmale …) oder „Leere Liste" (blanko Zeilen zum Ausfüllen, Zeilenanzahl wählbar). Oben wählst du die Klasse (oder „Alle Klassen") und optional einen Titel.',
      'Zusätzlich fügst du beschriftbare Leerspalten zum Ausfüllen hinzu: Bei „Anzahl" legst du fest, wie viele, und je Spalte gibst du optional eine Überschrift ein (leer lassen für ein blankes Kopffeld). Die Form wählst du mit „▫ Quadratisch" (schmale Kästchen – gut für viele Häkchen-Spalten) oder „▭ Breit" (breite Schreibfelder – gut für Notizen). Dazu optional eine Nr.-Spalte, die Ausrichtung (Hoch-/Querformat) und die Zeilenhöhe (Normal/Groß). „PDF erstellen" fragt den Speicherort ab.',
  ...
  },
  {
    id: 'kompetenzen', titel: 'Kompetenzen (Assistent & Netzdiagramm)', bloecke: [
      'Im Leistungsprofil kannst du pro Fach den Kompetenzstand einer:s Schüler:in mit einem Assistenten erfassen und die Entwicklung als Netzdiagramm verfolgen. Hinterlegt sind die Raster für Deutsch (Schulstufe 1–8), Mathematik (1–8), Englisch (5–8), die 1. lebende Fremdsprache der Primarstufe (1–4) sowie die Volksgruppensprachen Kroatisch/Slowenisch/Ungarisch (Schulstufe 3, 4, 7, 8).',
      'Ob ein Raster erscheint, erkennt Daskala am Fachnamen (z. B. „Deutsch", „Mathematik", „Englisch", „Französisch"). Heißt ein Fach anders – oder soll es als Volksgruppensprache geführt werden –, kannst du das Raster pro Fach zuordnen: Rechtsklick auf das Fach → „Kompetenzraster…" → Automatisch / Deutsch / Mathematik / Lebende Fremdsprache-Englisch / Volksgruppensprache / Keines. Kroatisch, Slowenisch oder Ungarisch werden automatisch als lebende Fremdsprache erkannt; für den Volksgruppen-Lehrplan wählst du hier „Volksgruppensprache".',
      { h: 'Assistent starten' },
      'Öffne das Profil einer:s Schüler:in (Klick auf den Namen), wähle links das Fach (Deutsch) und klicke im Abschnitt „Kompetenzen" auf „+ Assistent starten". Im ersten Schritt legst du Datum und optional einen Titel fest und wählst die Schulstufe (beim ersten Mal; danach ist sie für dieses Fach fixiert, damit die Achsen vergleichbar bleiben). Ab der 6. Schulstufe wählst du zusätzlich das Leistungsniveau „Standard (Mittelschule)" oder „Standard AHS" – die angezeigten Kann-Formulierungen richten sich danach. Bei „Standard (MS)" kennt das Raster nur ein Niveau („unter Anleitung"), d. h. je Kann-Beschreibung erreicht oder (noch) nicht; „Standard AHS" hat alle drei Niveaus. Ist beim Fach das differenzierte Benotungssystem (AHS/ST) aktiv, wird der Zweig automatisch aus dem Niveau der Schüler:in gesetzt (AHS → Standard AHS, ST → Standard MS).',
      'Danach führt dich der Assistent Bereich für Bereich durch das Raster. Innerhalb jedes Kompetenzbereichs – nach Teilkompetenz gegliedert – bewertest du jede einzelne Kann-Beschreibung: Du wählst je Beschreibung das erreichte Niveau, wobei die Optionen die Formulierungen des Rasters zeigen (z. B. „unter Anleitung …", „zunehmend selbstständig …", „selbstständig …"); „noch nicht erfasst" lässt sie offen. Bei den Volksschul-Stufen 1–2 gibt es nur ein Niveau („erreicht"). Zum Abschluss speicherst du – ein Durchlauf ergibt eine „Erhebung" zu diesem Zeitpunkt.',
      { h: 'Netzdiagramm & Verlauf' },
      'Das Netzdiagramm zeigt je Kompetenzbereich den Durchschnitt der erfassten Kann-Beschreibungen auf einen Blick (Skala nach den Niveaustufen der Schulstufe). Gibt es mehrere Erhebungen, wechselst du darunter über die Datums-Schaltflächen (oder die ‹ ›-Pfeile) zwischen den Erhebungszeitpunkten; das Diagramm wechselt animiert auf den jeweiligen Stand. Über „Details anzeigen" siehst du je Teilkompetenz das Ø-Niveau zum gewählten Zeitpunkt. So wird die Entwicklung über das Schuljahr sichtbar.',
      'Du kannst den Assistenten beliebig oft pro Jahr durchlaufen (z. B. je Semester). Einzelne Erhebungen lassen sich in der Liste unter dem Diagramm bearbeiten (✎, zum Korrigieren falscher Eingaben – Datum, Titel und alle Niveaus) oder löschen (✕).',
      { h: 'Im PDF-Export' },
      'Die Kompetenz-Netzdiagramme werden in den PDF-Export des Leistungsprofils (Kopfzeile → „PDF") übernommen – je Fach im jeweiligen Abschnitt. Gibt es Erhebungen, öffnet sich vor dem Speichern ein Auswahl-Dialog: Dort wählst du je Fach, welche Erhebungen als Diagramm aufgenommen werden (Standard: alle) – oder exportierst ohne Diagramme. Zur Orientierung sind die Achsen im PDF nummeriert; die zugehörigen Bereichsnamen stehen als Legende darunter.',
      ...
      { tipp: 'Für Fächer ohne hinterlegtes Raster erscheint der Kompetenzen-Bereich nicht. Nicht jedes Raster kennt drei Niveaus: Mathematik ist einstufig (je Kann-Beschreibung „erreicht" oder noch nicht), Volksgruppensprachen haben EINE Skala mit drei Niveaus für alle – bei beiden entfällt die Standard/AHS-Auswahl.' },
    ],
  },
  {
