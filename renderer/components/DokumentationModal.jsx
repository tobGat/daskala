...
const DOKU = [
  ...
  },
  {
    id: 'noten', titel: 'Noten eintragen', bloecke: [
      ...
      { ul: [
        ...
        'MA – Mitarbeit: zweistufig als + / − oder ↗ / ↘, vierstufig als Smiley-Skala (😄 😞), oder als „Note (1–5)" (echte Mitarbeitsnote)',
        'HÜ – Hausübung (✓ gemacht / ✗ nicht gemacht / — nicht gewertet)',
      ] },
      'Beim Anlegen einer Mitarbeits-Spalte wählst du die Bewertungsskala: „+ / −", „↗ / ↘" (nur eine andere Darstellung von + / −, gleiche Bewertung), „Smileys" mit vier Stufen – 😄 sehr fröhlich, 🙂 mäßig fröhlich, 🙁 mäßig traurig, 😞 sehr traurig – oder „Note (1–5)". Eine neue MA-Spalte übernimmt die zuletzt gewählte Variante. Bei Smiley-Spalten öffnet ein Klick auf die Zelle ein kleines Auswahlfeld.',
      'Die Skala „Note (1–5)" macht Mitarbeit zu einer echten benoteten Leistung („Mitarbeitsnote"): Sie zählt wie SA/Test/Individuell als vollwertige Note mit eigener Gewichtung und ist niveau-fähig (AHS/ST). Damit lässt sich auch in Fächern ohne Schularbeiten/Tests eine belastbare Zeugnisnote bilden. Die symbolische Mitarbeit (+/−, ↗/↘, Smileys) bleibt dagegen reiner Bonus/Malus.',
      'Bei der 4-stufigen Mitarbeit kannst du die Smileys je Spalte durch eigene Symbole ersetzen (z. B. +, +~, -~, -). Die Reihenfolge ist immer sehr positiv → sehr negativ, die Wertung entspricht genau den Smiley-Stufen. Ebenso lassen sich bei der Skala „Note (1–5)" die Ziffern durch fünf eigene Symbole ersetzen (Reihenfolge Note 1 → 5). Die Symbole legst du beim Anlegen der Spalte fest; intern zählt weiterhin die Position als Note. Beim Anlegen zeigt jede Bewertungsvariante beim Überfahren einen kurzen Info-Tooltip.',
      'Wie stark jeder Eintrag die Note verschiebt, legst du in den Einstellungen unter „Erweitert – Einfluss je Stufe" fest (Standard 0,1; Smiley „mäßig" 0,05). Die maximale Gesamt-Verschiebung („Deckelung") stellst du getrennt für Mitarbeit und Hausübung ein.',
      ...
      'Bei Schularbeiten, Tests, Individuell- und Mitarbeitsnoten-Spalten kannst du ein Thema hinterlegen (z. B. „Bruchrechnen"). Es erscheint als Tooltip am Spaltenkopf und im Leistungsdiagramm des Schüler:innen-Profils.',
  ...
  },
  {
    id: 'benotung', titel: 'Benotung verstehen', bloecke: [
      ...
      { h: 'Gewichtung' },
      'Die Note bilden SA, Test, Individuell und die Mitarbeitsnote (benotete Mitarbeit) – nach einer Gewichtung, die du global (Einstellungen) und pro Fach (Rechtsklick → Gewichtung) festlegst. Die symbolische Mitarbeit (+/−, Smileys) und Hausübungen zählen nicht als Note, sondern nur als Einfluss.',
      { h: 'Neuere Leistungen stärker gewichten (§ 20 LBVO)' },
      'Innerhalb einer Kategorie (SA, Test, Individuell) kannst du neuere Leistungen stärker gewichten als frühere – so verlangt es § 20 LBVO („zuletzt erreichter Leistungsstand"). Über den Rezenz-Faktor (Einstellungen → Benotung) legst du fest, wie viel stärker die neueste Note zählt: 1,0 = alle gleich (reiner Durchschnitt), 2,0 = neueste zählt doppelt so stark wie die älteste. Sortiert wird chronologisch über das ganze Jahr (nach Datum, semesterübergreifend). Beim ersten Start und nach dem einführenden Update wirst du einmalig gebeten, diesen Faktor festzulegen.',
      ...
      'Die Deckelung greift erst am Schluss: Alle Mitarbeits-Einträge werden zunächst zu einer Rohsumme addiert (bei der Smiley-Skala mit ihren Gewichten) und erst dann auf die maximale Verschiebung begrenzt. Viele Minus bleiben also „im Minus", bis genug Plus die Rohsumme wieder über die Grenze hebt.',
