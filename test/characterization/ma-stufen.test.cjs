...
function noteMitMA(maStufen, maWerte, opts = {}) {
...
test('Pfeil-Spalten (↗/↘) speichern +/− und rechnen identisch', () => {
  // ma_symbol = 'pfeil' ändert nur die Anzeige; Werte bleiben +/−, Berechnung wie 2-stufig.
  assert.strictEqual(noteMitMA(2, ['+'], { symbol: 'pfeil' }), 2.9)
  assert.strictEqual(noteMitMA(2, ['-'], { symbol: 'pfeil' }), 3.1)
})

test('Einfluss je Stufe frei konfigurierbar (Einstellungen)', () => {
  // Aufwärts-Gewicht auf 0,2 → + verbessert um 0,2
  assert.strictEqual(noteMitMA(2, ['+'], { gewichte: { ma_w_plus: 0.2 } }), 2.8)
  // Abwärts-Gewicht auf 0,3 → − verschlechtert um 0,3
  assert.strictEqual(noteMitMA(2, ['-'], { gewichte: { ma_w_minus: 0.3 } }), 3.3)
  // 🙂 auf 0,1 hochsetzen → wie 😄
  assert.strictEqual(noteMitMA(4, ['🙂'], { gewichte: { ma_w_smiley_pos: 0.1 } }), 2.9)
  // 😞 auf 0,2 → verschlechtert um 0,2 (unter Deckelung 0,5)
  assert.strictEqual(noteMitMA(4, ['😞'], { gewichte: { ma_w_smiley_vneg: 0.2 } }), 3.2)
})
